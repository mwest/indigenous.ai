// 001_baseline — the complete schema as of the Indigenous.ai Phase 0 release,
// including every guarded upgrade path accumulated during boot-time-migration
// development. Idempotent by construction: on a fresh database it builds the
// full current schema; on any historical database (back to day one) it fills
// exactly the gaps. It is therefore safe to run on any restored backup.
//
// transaction=false because the memberships/projects table rebuilds must
// toggle PRAGMA foreign_keys, which is a no-op inside a transaction; each
// rebuild wraps itself in its own transaction instead. This file is FROZEN —
// schema changes go in new numbered migrations.
...
`);
  ...
  // from the surviving work_item or audio row. Idempotent reconciliation (rows
  // whose work item AND audio are both gone stay NULL and the UI shows a
  // placeholder).
